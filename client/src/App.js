...
class App extends Component {
  ...
  handleBuyToken = async () => {
    await this.myTokenSale.methods.buyTokens(this.accounts[0]).send({from: this.accounts[0], value: 1000000});
  }
  updateUserTokens = async() => {
    let userTokens = await this.myToken.methods.balanceOf(this.accounts[0]).call();
    this.setState({userTokens: userTokens});
  }
  notifyTokenPurchased = async() =>{
    toast("You have purchased successfully 1 million tokens")
    this.updateUserTokens()
  }

  listenToTokenTransfer = async() => {
    this.myToken.events.Transfer({to: this.accounts[0]}).on("data", this.notifyTokenPurchased);
  }
  handleKycSubmit = async () => {
    const {kycAddress} = this.state;
    await this.kycContract.methods.setKycCompleted(kycAddress).send({from: this.accounts[0]});
    toast("Account "+kycAddress+" is now whitelisted");
  }
  runExample = async () => {
